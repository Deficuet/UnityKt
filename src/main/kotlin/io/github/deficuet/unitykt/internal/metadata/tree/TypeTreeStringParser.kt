...
internal class TypeTreeStringParser(
    ...
    override fun writeNode(node: TypeTreeNodeImpl) {
        builder.append("${indent(node)}${node.type} ${node.name}").append(lineSeparator)
    }

    override fun writeNodePrimitive(node: TypeTreeNodeImpl, value: Any) {
        builder.append("${indent(node)}${node.type} ${node.name} = $value").append(lineSeparator)
    }

    override fun writeString(node: TypeTreeNodeImpl, str: String) {
        builder.append("${indent(node)}${node.type} ${node.name} = \"${str}\"").append(lineSeparator)
    }

    override fun writeArrayHeader(node: TypeTreeNodeImpl, size: Int) {
        writeNode(node)
    ...
    override fun writeElementIndex(elementNode: TypeTreeNodeImpl, index: Int) {
        builder.append("${indent(elementNode)}[$index]").append(lineSeparator)
    }

    override fun flush() = builder.toString()

    private fun indent(node: TypeTreeNodeImpl): String {
